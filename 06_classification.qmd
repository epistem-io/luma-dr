...
2.  Model accuracy report if validation data available (including overall accuracy, precision, recall, F1-score, Kappa, model error matrix). This output is only available if the user has a validation data from the [splitted the sample data](@sec-data-split)
3.  Classified LULC raster map.

# Process {.unnumbered}

## Extracting spectral feature from sample data {#sec-sample-data-feature-extraction}
...
## Default classification scheme classification from probability layers

When the user [chose to use default classification scheme](@sec-user-select-existing-lulc-template), @sec-sample-data-feature-extraction and @sec-create-classification-model are skipped.

1.  The system loads the [pre-generated probability map](@sec-produce-prob-maps) stack and clips it to the user's AOI.
