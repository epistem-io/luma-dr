...
1.  The user is prompted to fill in the parameters for separability analysis. The system provides a default value for each of the parameters as the following:
    ...
    -   Maximum number of pixels which the separability analysis will be conducted to: 5000
2.  The system shows a message showing Transformed Divergence method will be used to conduct the analysis.

::: callout-important
## Feature Notes

There is no option for the user to change the separability analysis method.
:::
...
3.  The user is presented with an overall summary of the separability analysis, including the separability category (good, weak, or poor) for each class and a highlight of which LULC classes require improvement, based on the output of @sec-caluclate-separability-index. A dataframe containing details of the pairwise seaparability is also displayed.
...
1.  This step does not involve operations from Luma Geospatial Engine.

    ::: callout-important
    ## Feature Notes

    All the spectral features plot visualizations were made on Streamlit script.
    :::