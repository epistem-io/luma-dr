...
1.  The system validates the uploaded `.csv` to ensure IDs are numeric and unique, applies a set of distinct colors if no color code was provided by the user. The user can change the color code manually.
...
1.  The system validates user-provided class input for numeric ID, non-empty class name, and uniqueness (if adding new classes).
    ...
    Known Issue: `add_class()` permits duplicate class IDs when operating in edit mode.

    Duplicate ID validation is only enforced when `self.edit_mode` is `False`. As a result, a user can unintentionally create multiple classes with the same ID by editing an existing class and changing its ID to one that already exists.
    :::
