...
2.  The system retrieves images that match user’s selected criteria, including spatial resolution, year, and cloud cover based on the following input parameters: AOI, target map date, and cloud cover threshold.
    ...
    -   The system performs cloud masking with QA_PIXEL band that encodes pixel conditions using specific bit flags. Bitwise AND operation are applied to isolate these conditions, focusing on clouds (bit 4, value 16) and cloud shadows (bit 3, value 8). Confidence threshold of these features is applied, in which only pixels with high confidence is selected as the final mask. Pixels matching the final mask, is excluded to produce a cleaner image across the image collection.
