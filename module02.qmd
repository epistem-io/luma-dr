# Determining LULC Classification Scheme and Classes {#sec-module-2}
...
-   If the user chooses a template for the classification scheme, the system proceeds to @sec-user-select-existing-lulc-template
...
1.  The system automatically detects column headers based on common naming conventions for ID, class name, and color code. If the expected headers are not found, the user must manually assign the correct column names.
    ...
    `classification_scheme.LULC_scheme_Manager.auto_detect_csv_columns()`: this function automatically detects the column headers. The set for the common naming conventions are as the following:
...
`classification_scheme.LULC_scheme_Manager.edit_class()` `classification_scheme.LULC_scheme_Manager.delete_class()` `classification_scheme.LULC_scheme_Manager.cancel_edit()`
:::

::: callout-important
## Feature Note

The related functions of this step are invoked inside the `render_manual_input_form()` function on Streamlit development.
:::

### User Selecting an Existing Classification Scheme {#sec-user-select-existing-lulc-template}

**Luma User Journey**

1.  The user is prompted to select an existing classification scheme.

::: callout-important
## Feature Notes

In the current development, only classification scheme from RESTORE+ is available.
:::
