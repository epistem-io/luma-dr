...
2. Service account authentication is the default for deployments and CI. It is non-interactive and repeatable, but it is **not a good fit for exporting to a user’s personal Google Drive**, because a service account is not a normal Drive user.

A third option, OAuth2 login in the interface, is not implemented yet. It is mentioned here because it is a plausible path to per-user Drive exports and per-user attribution without sharing credentials.
...
::: callout-warning
Luma-GE does not support running manual authentication inside a Streamlit interface. Earth Engine’s interactive flow expects terminal/browser control that a hosted app generally does not have. If you need “login” inside a UI, that is an OAuth2 web-app feature (see 4c).
:::
...
### 4c. OAuth2 user login (not implemented yet)

OAuth2 user login means each user authorises Earth Engine using their own Google account through the interface (a “Login with Google” button). This is the cleanest way to support user-owned Drive exports and avoid shared credentials.

This is not implemented in Luma-GE yet. Doing it properly requires a secure web OAuth2 flow (redirects, token handling, refresh, logout, secure storage), tighter integration between the UI stack and Luma-GE, and explicit decisions about what the interface is allowed to do on behalf of the user.

As an existence proof that an Earth Engine-backed web interface can work with OAuth2 user login is the Remap app: [https://remap-app.org/](https://remap-app.org/)
